...
check_new_ver(){
	new_ver=$(wget -qO- https://api.github.com/repos/zhucaidan/mtg/releases| grep "tag_name"| head -n 1| awk -F ":" '{print $2}'| sed 's/\"//g;s/,//g;s/ //g')
...
Download(){
	...
	wget --no-check-certificate -N "https://github.com/zhucaidan/mtg/releases/download/${new_ver}/mtg"
	[[ ! -e "mtg" ]] && echo -e "${Error} MTProxy 下载失败 !" && rm -rf "${file}" && exit 1
	chmod +x mtg
	echo "${new_ver}" > ${Now_ver_File}
}
Service(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/zhucaidan/doubi/master/service/mtproxy_go_centos" -O /etc/init.d/mtproxy-go; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/zhucaidan/doubi/master/service/mtproxy_go_debian" -O /etc/init.d/mtproxy-go; then
			echo -e "${Error} MTProxy服务 管理脚本下载失败 !"
			rm -rf "${file}"
			exit 1
		fi
